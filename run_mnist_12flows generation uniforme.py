...
# ============================================================
# Chemins Kaggle - source uniforme copule indépendante + projections positives
# ============================================================
SAVE_DIR = "/kaggle/working/mnist_swot_flow_flows14_bs256_lr3e5_proj2000_uniform_copula_positive_lamb85e7_gamma33e9"
OUTDIR = "/kaggle/working/mnist_swot_flow_flows14_bs256_lr3e5_proj2000_uniform_copula_positive_lamb85e7_gamma33e9/mnist_result"
...
# ============================================================
# Source uniforme indépendante : copule indépendante sur [0,1]^dim
# ============================================================

def sample_uniform_copula_noise(batch_size, dim, device):
    """
    Source uniforme indépendante sur [0,1]^dim.
    Elle peut être interprétée comme une copule uniforme indépendante.
    """
...
def save_generated_images(model, device, epoch, fixed_noise, outdir=OUTDIR, display_result=True):
    ...
    with torch.no_grad():
        ...
        # Les images générées sont attendues dans [0, 1],
        # car la cible MNIST n'est plus normalisée dans [-1, 1].
...
def save_comparison_images(
    ...
    with torch.no_grad():
        ...
        # Les vraies images et les images générées sont dans [0, 1].
...
def main():
    ...
    # ========================================================
    # Hyperparamètres : test 14 flows, source uniforme copule indépendante
    # ========================================================
    ...
    fixed_noise = sample_uniform_copula_noise(64, dim, device)
    # ========================================================
    # Dataset MNIST
    # ========================================================
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        for images, _ in loader:
            ...
            source = sample_uniform_copula_noise(current_batch_size, dim, device)
    ...
    model_path = os.path.join(SAVE_DIR, "mnist_swot_flow_flows14_bs256_lr3e5_proj2000_uniform_copula_positive.pth")
